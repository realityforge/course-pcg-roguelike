...
public class WorldConfigurationBuilder
{
    ...
    throws ReflectionException
  {
    ...
    if ( ClassReflection.isAbstractClass( type ) )
    {
      ...
      {
        throw new WorldConfigurationException( "Implementation of " +
                                               type +
                                               " expected but not found. Did you forget to include a plugin? (for example: logging-libgdx for logging-api)" );
      }
    }
    else
    {
  ...
  private void addSystems( final int priority, @Nonnull final BaseSystem[] systems )
  {
    for ( final BaseSystem system : systems )
    {
      ...
      {
        throw new WorldConfigurationException( "System of type " +
                                               system.getClass() +
  ...
  private void addPlugins( @Nonnull final ArtemisPlugin[] plugins )
  {
    for ( final ArtemisPlugin plugin : plugins )
    {
      ...
      {
        throw new WorldConfigurationException( "Plugin of type " +
                                               plugin.getClass() +
                                               " registered twice. Only once allowed." );
      }
